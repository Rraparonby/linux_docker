...
source "../../../source_docker/lib_lote/02_mariadb_springboot_react/mariadb_springboot_react_lib_lote.sh"
...
get_menu_opciones_lote() {
    clear
    echo
    echo -e "\t-----------  LOTE (MARIADB - SPRING - SPRING) - HOST-PC "
    ...
    echo -e "\t-----------------------------------------"
    echo -e "\t2. Ejecutar Comando Container SPRING Interactivo GENERAL Colegio (docker exec -it)"
    echo -e "\t3. Ejecutar Comando Container SPRING Parametro GENERAL Colegio (docker exec -it)"
    echo -e "\t4. CORRER Comando Container MARIADB PARAMETRO Colegio (docker run -it)"
    echo -e "\t5. CORRER Comando Container SPRING PARAMETRO Colegio (docker run -it)"
    echo -e "\t6. CORRER Comando Container REACT PARAMETRO Colegio (docker run -it)"
...
do
    ...
    case $opcion_lote in
        ...
        5)  exec_run_spring_lote ;;
        6)  exec_run_react_lote ;;
